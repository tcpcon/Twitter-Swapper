...
func releaseUsername(newName string, rareAccount Account) bool {
	...
	req.Header.Set("authorization", "Bearer " + rareAccount.Authorization)
...
func claimUsername(target string, recievingAccount Account) {
	...
	req.Header.Set("authorization", "Bearer " + recievingAccount.Authorization)
...
func Swap(target string, receivingAccount Account, rareAccount Account) SwapStatistics {
	...
	go func() {
		for {
			newName = randomLetterNumberString(10)
			completed := releaseUsername(newName, rareAccount)
	...
	var swapStatistics SwapStatistics
	swapStatistics.TimeElapsed = globals.TimeElapsed
	swapStatistics.RareAccountNewName = newName
	swapStatistics.ReceivingAccountNewName = target
